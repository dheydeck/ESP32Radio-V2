...
// Includes for various ST7735 displays.  Size is 320 x 240.                                       *
// Lilygo TTGO TM Music Albums with 320 x 240 ST7789 Display, requires colours to be inverted.     *
//**************************************************************************************************
#include "ttgo-tm.h"

Adafruit_ST7789*     TTGOTM_tft ;                          // For instance of display driver
scrseg_struct        TTGOTM_tftdata[TFTSECS] =             // Screen divided in 3 segments + 1 overlay
...
{
  if ( ( TTGOTM_tft = new Adafruit_ST7789 ( cs, dc , -1 ) ) )      // Create an instant for TFT
  {
    dsp_init() ;
    dsp_invert(); // DGH For TTGO TM Music Albums
  }
  return ( TTGOTM_tft != NULL ) ;
}
  ...
  if ( TTGOTM_tft )
  {
    if ( bat0 < bat100 )                                  // Levels set in preferences?
    {
      ...
      if ( newpos != oldpos )                             // Value changed?
      {
        ...
        ypos = TTGOTM_tftdata[1].y - 5 ;                  // Just before 1st divider
...
void TTGOTM_displayvolume ( uint8_t vol )
{
  if ( TTGOTM_tft )
  {
    ...
    if ( vol != oldvol )                                // Volume changed?
    {
      ...
      // Following heights increased from 2 to 4 for better visibility
...
//**************************************************************************************************
void TTGOTM_displaytime ( const char* str, uint16_t color )
{
